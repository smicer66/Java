...
public class CategoryList extends javax.swing.JPanel {
     ...
     */
    public CategoryList(Mysql mysql, Connection con, Dimension dimension, String installPath, HomeScreen hs) {
        this.con=con;
        this.mysql=mysql;
    ...
    public JPanel getAllComponents()
    {
        ...
        try {
            ...
            if(this.parentId>0)
            {
                pr3 = this.con.prepareStatement("Select * from asset_category WHERE id= ?");
                pr3.setInt(1, this.parentId);
                rs1=this.mysql.query(pr3);
                while (rs1.next())
                {
                    jLabel8.setText("Asset Categories under " + rs1.getString("categoryName") + " category.");
                }
            }
            else
            {
                jLabel8.setText("All Asset Categories");
            }
            ...
            if(this.parentId>0)
            {
                pr3 = this.con.prepareStatement("Select * from asset_category WHERE parentId = ?");
                pr3.setInt(1, this.parentId);
            }
            else
            {
                pr3 = this.con.prepareStatement("Select * from asset_category");
            }
            ...
            while (rs1.next()) { 
               // System.out.println(" File type = " + rs1.getString("fileType"));
                ...
                {
                    jLabelTmp.setIcon(new javax.swing.ImageIcon(getClass().getResource("/shopscanmanager/assets.png")));
                }
                else 
                {
                    ...
                    bufferedImageQR=new PictureManager().getPicturePath(installPath + "\\asset_category\\", 
                ...
                jLabelTmp.setToolTipText(rs1.getString("categoryName"));
                ...
                m.add(check);
                JLabel lbl=new JLabel(rs1.getString("categoryName"));
                lbl.setForeground(new Color(204,204, 204));
                lbl.setBackground(new Color(23,21, 21));
                m.add(lbl);
                jPaneltmp.add(m);
                jPaneltmp.addMouseListener(new MouseListener()
                {
                    ...
                    public void mouseClicked(MouseEvent e) {
                        try
                        {
                            ...
                            if(clickCount==1)
                            {
                                CategoryList.this.hs.displayCategoryList(rs1.getInt("id"));
                                System.out.println("Clicking on id = " + rs1.getInt("id"));
                            }
                        }
    ...
    private void initComponents() {
        ...
        jLabel8.setText("Asset Categories Saved");
